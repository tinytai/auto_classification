...
def readJpg2Array(path):
    ...
    arrHorse=np.asarray(arrHorse,dtype=np.float32)
    arrCat=np.asarray(arrCat,dtype=np.float32)
    imgPair=cv.LoadImage(path,0)

    for h in range(164):
        for w in range(164):
            pixPair=cv.Get2D(imgPair,h,w)
            arrTemp=np.array(pixPair)
            arrPair[0,w,h,0]=arrTemp[0]
...
if __name__ == '__main__':
    ...
    temp_X_train=randn(3,164,164,1)
...
if flag == "1":
    for i in xrange(8):   
        for k in xrange(46):
            temp_X_train[0,:,:,0]= X1_train[k,:,:,0]#pair
            temp_X_train[1,:,:,0]= X2_train[k,:,:,0]#horse
            temp_X_train[2,:,:,0]= X3_train[k,:,:,0]#cat
            #print 'traing ...k' + str(k)
    ...
    save_path = saver.save(sess, "self.ckpt")
    #out = readImage2Array("/home/deeplearn/Desktop/qin/testSample/2.bmp")
    ...
    for i in xrange(9):
        out = readImage2Array("/home/deeplearn/Desktop/qin/testSample/"+str(i)+".bmp")
        #out = readJpg2Array("/home/deeplearn/Desktop/qin/cat_46/"+str(i)+".jpg")
	print "out.shape"
        print out.shape
        #y_batch = y_conv.eval(feed_dict={x:out[0:1,:,:,:], keep_prob:1.0})
        y_batch = y_conv.eval(feed_dict={x:out, keep_prob:1.0})
        carConfig=readCarConfig("/home/deeplearn/Desktop/qin/result.txt")
        result = carConfig[y_batch[0].argmax()]
        print result, i
